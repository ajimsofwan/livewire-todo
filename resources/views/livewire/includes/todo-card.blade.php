<div wire:key='{{ $todo->id }}'
  class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 {{ $todo->completed ? 'border-green-500' : 'border-blue-500' }} hover:shadow">
  <div class="flex justify-between space-x-2">

    @if ($updateId === $todo->id)
      <div>
        <input wire:model='updateName' type="text" placeholder="Todo.."
          class="bg-gray-100  text-gray-900 text-sm rounded block w-full p-2.5 @error('updateName') border border-red-500 @enderror">
        @error('updateName')
    ...
    @else
      <div class="flex items-center">
        @if ($todo->completed)
          <input wire:click='toggle({{ $todo->id }})' type="checkbox" name="completed" id="completed"
            class="mr-2 w-4 h-4 text-green-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500 focus:ring-2
            checked>
@else
...
@endif
        ...
        <h3 class="text-lg
            text-semibold text-gray-800">{{ $todo->name }}</h3>
      </div>
